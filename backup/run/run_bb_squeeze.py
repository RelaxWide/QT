"""
Bollinger Squeeze + Volume Breakout 백테스트
사용법: python run_bb_squeeze.py [--refresh]
"""
import argparse
import time
...
from src.strategy.bb_squeeze import generate_bb_squeeze_signals
...
def main():
    ...
    s_cfg = cfg.get("bb_squeeze_strategy", {})
    ...
    t0 = time.time()
    data = fetch_all(tickers, start, end, min_bars=280, refresh=args.refresh)
    print(f"  {len(data)} loaded in {time.time()-t0:.1f}s")

    spy_close = data["SPY"]["close"]

    print("Generating BB Squeeze signals...")
    t0 = time.time()
    signals = []
    for sym, df in data.items():
        if sym == "SPY":
            continue
        signals.extend(generate_bb_squeeze_signals(sym, df, s_cfg))
    ...
    t0 = time.time()
    equity, trades = run_swing_backtest(signals, data, spy_close, cfg, "bb_squeeze_strategy")
    print(f"  Done in {time.time()-t0:.1f}s | {len(trades)} trades")

    m = compute_metrics(equity, trades, cap)

    print("\n── BB Squeeze + Volume Results ──────────────────────")
    for k, v in m.items():
        print(f"  {k:30s}: {v}")
