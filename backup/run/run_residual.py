"""
Residual Momentum 백테스트
사용법: python run_residual.py [--tickers AAPL MSFT ...] [--refresh]

잔차 모멘텀 = 총 수익에서 SPY 베타 기여분을 제거한 개별주 고유 모멘텀.
S&P500 전체 실행 시 OLS 계산으로 수 분 소요.
"""
import argparse
import time
...
from src.backtest.residual_momentum_engine import (
    run_residual_backtest,
...
def main():
    ...
    rm_p  = cfg.get("residual_strategy", {})

    tickers = args.tickers or get_sp500_tickers()
    if "SPY" not in tickers:
        tickers = ["SPY"] + list(tickers)

    beta_window = rm_p.get("beta_window", 252)
    lookback    = rm_p.get("lookback", 252)
    ma100_p     = rm_p.get("ma100_period", 100)
    min_bars    = beta_window + lookback + ma100_p + 10
    ...
    print("Running Residual Momentum backtest (weekly rebalancing)...")
    t0 = time.time()
    equity, rebal_log = run_residual_backtest(price_data, cfg)
    print(f"  Done in {time.time()-t0:.1f}s")

    m = compute_residual_metrics(equity, cap)
