"""
OBV New High Dual Confirmation 백테스트
사용법: python run_obv_newhi.py [--refresh]
"""
import argparse
import time
...
from src.strategy.obv_newhi import generate_obv_newhi_signals
...
def main():
    ...
    s_cfg = cfg.get("obv_newhi_strategy", {})
    ...
    print("Generating OBV New High signals...")
    t0 = time.time()
    signals = []
    for sym, df in data.items():
        if sym == "SPY":
            continue
        signals.extend(generate_obv_newhi_signals(sym, df, s_cfg))
    ...
    t0 = time.time()
    equity, trades = run_swing_backtest(signals, data, spy_close, cfg, "obv_newhi_strategy")
    print(f"  Done in {time.time()-t0:.1f}s | {len(trades)} trades")

    m = compute_metrics(equity, trades, cap)

    print("\n── OBV New High Results ─────────────────────────────")
    for k, v in m.items():
        print(f"  {k:30s}: {v}")
