"""
Darvas Box 백테스트
사용법: python run_darvas.py [--refresh]
"""
import argparse
import time
...
from src.strategy.darvas_box import generate_darvas_signals
...
def main():
    ...
    print("Generating Darvas Box signals...")
    t0 = time.time()
    signals = []
    for sym, df in data.items():
        if sym == "SPY":
            continue
        signals.extend(generate_darvas_signals(sym, df, s_cfg))
    ...
    t0 = time.time()
    equity, trades = run_swing_backtest(signals, data, spy_close, cfg, "darvas_strategy")
