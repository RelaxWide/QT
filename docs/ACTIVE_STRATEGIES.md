...
> 최종 업데이트: 2026-05-15
> 지원 시장: **US (S&P 500)** ✅ 실전 운영 / **KR (KOSPI200)** ✅ 11년 백테스트 통과, KIS 모의 검증 대기
...
| 전략 | 시장 | 유형 | 기간 | CAGR | MDD | Sharpe | 상태 |
|---|---|---|---|---|---|---|---|
| **Clenow Momentum** | US | 주봉 모멘텀 | 2015~ | 16.92% | -19.6% | 1.10 | ✅ 페이퍼 + **실계좌 운영 (2026-05-18~)** |
| **Clenow Momentum** | KR | 주봉 모멘텀 | 2015~ | **12.73%** | -20.0% | 0.86 | ✅ 11년 백테스트 3/4 게이트 통과, KIS 모의 대기 |
| **Weinstein Stage 2** | US | 주봉 추세 | 2015~ | 8.0% | -14.0% | 0.88 | ✅ 페이퍼 + **실계좌 운영 (2026-05-18~)** |
| Weinstein Stage 2 | KR | 주봉 추세 | 2015~ | 4.19% | -19.3% | 0.40 | ⚠️ KR 박스권 가짜 돌파 다발, PF 1.03 — KR 보류 |
| **합성 60:40** | US | 포트폴리오 | — | — | — | — | ✅ 페이퍼 NAV + 실계좌 동일 비율 |
| Phase 4 | US | 일봉 추세 | 2015~ | ~8% | -11.92% | 0.74 | ⚠️ Sharpe 미달, 페이퍼만 (실계좌 배분 0) |
...
| Phase | 내용 | 상태 |
|---|---|---|
| **1: 데이터 + 유니버스** | PyKRX/FDR 데이터, KOSPI200 유니버스, KR regime filter | ✅ 검증 완료 |
| **2: 백테스트** | KR 시장 백테스트 엔진, 거래세 0.18% 비용 모델 | ✅ 11년 실행 완료 (Clenow 채택, Weinstein 보류) |
...
데이터: PyKRX/FDR, 유니버스 KOSPI200 200종목, 비용 0.015% 매수 + 거래세 0.18%·농특세 0.023% 매도.

| 지표 | Clenow KR | Weinstein KR | KOSPI Buy&Hold |
|---|---|---|---|
| CAGR | **12.73%** | 4.19% | 13.69% |
| MDD | **-20.01%** | -19.26% | ~-55% |
| Sharpe | **0.86** | 0.40 | — |
| Sortino | 0.87 | 0.43 | — |
| Calmar | 0.64 | 0.22 | — |
| Profit Factor | — | 1.03 | — |
| Win rate | — | 21% | — |
| Monthly WR | 34.6% | 37.5% | — |
| Total return | +277% | +57.6% | +313.76% |
| 평균 보유 종목 | 6.8 | — | — |
| 거래 수 | — | 248 | — |

**Clenow KR**: KOSPI 와 유사한 수익률에 **MDD 절반 이하** (-20% vs -55%). Sharpe 0.86 = 위험조정 수익 우수.
KR 게이트 (CAGR≥8% / MDD≥-25% / Sharpe≥0.6 / Monthly WR≥50%) 중 **3/4 통과** (Monthly WR 만 미달 — 모멘텀 본연의 특성, 큰 winner 가 매월 못 나옴).
→ **채택 + KIS 모의 검증 진행**.
...
→ **KR 트랙에서 보류** (US 트랙은 계속 운영). KR Weinstein 활성화 시 거래량 멀티플 2.0 → 2.5, 52주 고가 근접도 강화 등 파라미터 재최적화 필요.
