...
> 최종 업데이트: 2026-05-19 (18년 백테스트 + Sensitivity 10×10 + DART 검증 완료 — KW SV 단일 채택 확정)
> 지원 시장: **US (S&P 500)** ✅ 실전 운영 / **KR** ✅ KW Super Value 단일 채택 (18년 CAGR 28~31% 검증)
...
| **KW Super Value** ⭐ | KR | 분기 가치 | **2008~** | **28.32% / 30.79%** | -48.2% | **1.23 / 1.30** | +20.13%p / **+27%p** | ✅ **KR 메인 엔진 확정** (18년 검증 통과) |
| ~~KW Super Quality~~ | ~~KR~~ | — | — | — | — | — | — | ❌ **폐기 확정** (2026-05-19, SV 와 중복 + DART 강화 효과 없음) |
...
### 10.2-N 18년 장기 백테스트 (2008-01-01 ~ 2026-05, 2026-05-19)

기존 12년 (2014~) 한계 극복 — **yfinance 폴백 추가로 2008년 데이터 사용 가능**. PyKRX/FDR 가 2015년 이전 KOSPI 개별 OHLCV 미제공 → yfinance batch download 로 838종목 일괄 fetch (`src/fetch/prices_kr.py` 의 `_fetch_yfinance` + `_yfinance_batch`).

#### 결과 — KW Super Value 18년 (small_cap=0.15, top_n=15)

| 지표 | 18년 전체 (2008~2026) | **18년 컷오프 (2008~2025-02)** | 12년 (2014~2026) |
|---|---|---|---|
| 기간 | 18.4년 | **16.6년** | 11.4년 |
| **CAGR** | +28.32% | **+30.79%** ⭐ | +20.71% |
| **Sharpe** | 1.23 | **1.30** ⭐ | 0.97 |
| Sortino | 1.71 | **1.81** | 1.32 |
| Calmar | 0.59 | 0.64 | 0.52 |
| MDD | -48.16% | -48.16% (2008 위기) | -39.17% |
| 누적 수익률 | **+8,324%** | **+9,400%** (추정) | +642% |
| 알파 vs KOSPI | +20.13%p | **~+27%p** | +6.51%p |
| 게이트 | 4/5 (MDD) | 4/5 (MDD) | 5/5 PASS |
| **14개월 상승장 기여** | — | **-2.42%** ⚠️ | +0.31% |

**충격적 발견**:
- **상승장 기여 -2.42%** — 14개월 KOSPI +196.75% 상승장에서 KW SV 는 **오히려 손실**
- **컷오프 후 CAGR 30.79% > 전체 28.32%** — 상승장이 성과를 깎아먹음
- **anti-correlation with market rally** — KW SV 의 알파는 시장과 반대 방향
- 2025-03~ 강세장 (대형주·성장주 주도) 에서 KW SV (소형 가치) 외면받음 → 그래도 17년 평균 30% CAGR

**MDD -48% 해석**:
- 2008 글로벌 금융위기 (KOSPI -55%) 단일 사건
- 2009 회복 후 16년간 알파 폭발적 유지
- 일회성 위기 vs 17년 연평균 30% CAGR — 위험 대비 수익 매우 우수

### 10.2-O Sensitivity 10×10 = 100 조합 (2026-05-19)

기존 5×4=20 조합 대비 정밀화. **Sharpe Heatmap**:

```
top_n           5    8    10   12   15   18   20   25   30   40
small_cap_pct
0.15          0.70 0.85 0.89 0.98 1.18 1.26 1.28 1.27 1.29 1.29  ⭐ sweet spot
0.18          0.79 0.70 0.81 0.91 0.91 0.92 0.97 1.11 1.16 1.16
0.20          0.67 0.89 0.82 0.82 0.91 0.98 1.07 1.02 1.05 1.20
0.25          0.62 0.81 0.79 0.78 0.75 0.75 0.79 0.81 0.86 0.92
0.30          0.54 0.63 0.67 0.69 0.69 0.65 0.68 0.73 0.74 0.81
```

**최적 파라미터 Top 5 (Sharpe)**:

| small_cap | top_n | CAGR | MDD | Sharpe | Calmar | alpha |
|---|---|---|---|---|---|---|
| 0.15 | 40 | 28.88% | -36.46% | **1.29** | 0.79 | +20.78%p |
| 0.15 | 30 | 29.07% | -36.46% | 1.29 | 0.80 | +20.97 |
| 0.15 | 20 | 31.43% | -36.45% | 1.28 | 0.86 | +23.33 |
| 0.15 | 25 | 29.62% | -36.46% | 1.27 | 0.81 | +21.52 |
| **0.15** | **18** ⭐ | **32.01%** | -36.45% | 1.26 | **0.88** | **+23.91** |

**핵심**:
- **scp=0.15 가 sweet spot** 확정 (이전 12년 분석과 일치)
- top_n=18~40 모두 비슷한 성능 (분산 효과 + 안정성)
- **현재 0.15/15 (Sharpe 1.18) → 0.15/18 (Sharpe 1.26, Calmar 0.88) 업그레이드 권장**
- scp ≥ 0.30: 알파 급락 (소형주 효과 핵심 확인)
- scp ≤ 0.10: 데이터 부족, CAGR 11~18% 만

**12년 컷오프 + sensitivity 결합 권장 파라미터**:
- **small_cap_pct = 0.15**
- **top_n = 18** (Calmar 최고)
- **rebalance: [5, 8, 11, 4] / [16, 16, 16, 1]** (변경 없음)

### 10.2-P DART OpenAPI 강화 — Super Quality 효과 없음 (2026-05-19)

DART 838종목 × 10년 panel fetch 완료 (`data/raw/kr/dart_panel/{year}.parquet`, 600~764 종목/년).

#### Quality DART 가중치 실험

| 조합 (ROE:GP/A:Vol) | CAGR | MDD | Sharpe | Calmar | 게이트 |
|---|---|---|---|---|---|
| **PyKRX-only (기준)** | **18.92%** | -44.82% | **0.91** | 0.42 | **5/5** ✅ |
| DART 기본 1:1:1 | 16.70% | -47.31% | 0.80 | 0.35 | 3/5 |
| DART GP/A 강조 0.5:2:1 | 18.06% | -45.11% | 0.84 | 0.40 | 4/5 |
| DART ROE 강조 2:0.5:1 | 17.08% | -45.28% | 0.78 | 0.38 | 3/5 |
| DART GP/A 제외 1:0:1 | 17.30% | -46.49% | 0.82 | 0.37 | 3/5 |

**모든 DART 조합 < PyKRX-only** — DART 강화 효과 없음.

**원인**:
- DART universe (700~764) < PyKRX universe (838) → 후보 풀 작아짐
...
| 비교 | KW Super Value (18년 컷오프) | KW Super Quality (PyKRX-only) |
|---|---|---|
| 검증 기간 | **17년** (2008~2025-02) | 12년 (2014~2026) |
| CAGR | **30.79%** | 18.92% |
| Sharpe | **1.30** | 0.91 |
| Sortino | **1.81** | 1.13 |
| MDD | -48.16% | -44.82% |
| 상승장 의존도 | **-2.42%** ⭐ | +13.70% |
| SV 와 상관 | — | **0.91** (중복 큼) |
| KOSPI 대비 알파 | **+27%p** | +10.82%p |

→ **Super Quality 의 단독 가치 없음**: SV 의 절반 알파 + 0.91 상관 + 상승장 의존
→ **KW Super Value 단일 운영** 이 합리적

#### 폐기 / 보류 전략 최종 (2026-05-19 update)

| 전략 | 결정 |
|---|---|
| **KW Super Value** | ✅ KR 메인 엔진 (small_cap=0.15, top_n=18) |
| ~~KW Super Quality~~ | ❌ 폐기 (REJECTED 이동, 2026-05-19) |
| ~~Clenow KR~~ | ❌ 폐기 확정 (2026-05-18) |
| ~~KW Ultra~~ | ❌ 폐기 (Super Value 미만) |
| ~~Weinstein KR~~ | ❌ 폐기 (KR 박스권 가짜 돌파) |
| ~~DART GP/A 강화~~ | ❌ 폐기 (PyKRX-only 보다 못함) |
...
- ~~**KW Super Quality 채택 여부**~~ — **확정: 폐기** (2026-05-19, DART 강화 효과 없음 + SV 와 중복 0.91)
- **KR 실전 시드 자본** — 백테스트 5천만 → 실전 시작은? (1천만 → 분기 25% 증액 권장)
- **자본 배분 통합 운영 vs 시장 분리** — 미국·한국 단일 계좌 합산 NAV vs 별도

### 11.5 KW Super Value 실전 진입 가이드 (2026-05-19)

#### 운영 사양 (확정)

| 항목 | 값 |
|---|---|
| 전략 | KW Super Value (강환국 슈퍼가치) |
| 시장 | KOSPI 전체 (838 종목 universe) |
| 빈도 | 분기 4회 (5/16, 8/16, 11/16, 4/1, 영업일 보정) |
| 유니버스 필터 | 시총 하위 15% (`small_cap_pct=0.15`) |
| 보유 종목수 | top 18 동일비중 (`top_n=18`) |
| 매수 시점 | 리밸런싱일 09:00 KST 시초가 |
| 매도 시점 | 다음 리밸런싱일 (탈락 종목) |
| 손절 | 없음 (분기 사이 ffill 보유) |
| 호가단위 | `tick_size_kospi(price)` 자동 라운딩 |
| 거래세 | 매도 0.218% + 매수 0.015% |

#### 단계별 진입 (1년 timeline)

| 시점 | Phase | 자본 | 액션 |
|---|---|---|---|
| **5/16 (D-day)** | 페이퍼 1분기 시작 | — | `run_daily_kr.py --print-only` 로 신호 확인 |
| 5/16~8/15 | 페이퍼 운영 | NAV 추적 | 백테스트 신호와 일치 검증 |
| **8/16** | KIS 모의 진입 | 모의 자본 | KIS 한국주식 모의 신청 완료 가정 |
| 8/16~11/15 | KIS 모의 1분기 | 모의 | 슬리피지 < 0.5%, 미체결 < 10% 확인 |
| **11/16** | 실전 5% 진입 | **₩1,000,000** | 백테스트 5천만 × 5% 시드 |
| 11/16~2/15 | 실전 1분기 | 1천만 | 페이퍼 ±3%p 일치 확인 |
| **다음 분기** | 단계적 확대 | +₩2,500,000/분기 | 4분기 후 5천만 도달 |

#### 첫 분기 사용자 액션 체크리스트

**페이퍼 단계 (즉시)**:
- [ ] `config_live.yaml` 의 `kr.mode: "mock"` 확인
- [ ] `paper_trading/live_signals_kw.py` 구현 (다음 작업)
- [ ] `run_daily_kr.py` 에 KW SV 통합 (다음 작업)
- [ ] 5/16 09:00 KST 페이퍼 첫 신호 확인 → 텔레그램 알림

**모의 단계 (D+3개월)**:
- [ ] KIS 한국주식 모의계좌 신청 (한국투자증권 홈페이지)
- [ ] `kr_mock_account_no` 입력
- [ ] 8/16 09:00 KST 모의 매수 자동 실행 확인

**실전 단계 (D+6개월)**:
- [ ] `config_live.yaml` 의 `kr.mode: "prod"` 전환
- [ ] `kr_prod_account_no` 입력 (보통 `us_prod_account_no` 와 동일 종합계좌)
- [ ] 11/16 09:00 KST 실전 매수 자동 실행 확인
- [ ] 한국주식 거래정지/관리종목 사전 필터 작동 확인

#### 예상 성과 (백테스트 기반)

자본 ₩10,000,000 기준 4분기 후:
- 18년 평균 CAGR 30.79% → 1년 후 ₩13,079,000 (+30.79%)
- Sharpe 1.30 → 위험 대비 수익 매우 우수
- MDD -48% 시 ₩5,200,000 (2008 위기 시나리오)
- 일반 약세장 MDD -30% → ₩7,000,000

#### 위험 한도 (멘탈 가드)

| 시나리오 | 손실 규모 (₩1,000만 기준) | 대응 |
|---|---|---|
| 일반 분기 -5% | -50만 | 계속 운영 |
| 분기 -10% MDD | -100만 | 계속 운영 (백테스트 범위) |
| 1년 -20% | -200만 | 페이퍼 vs 실전 격차 ±5%p 확인 |
| 2008급 -45% | -450만 | 일생에 1번 — 견디기 |
| 페이퍼와 ±5%p 초과 격차 | — | 일시 중단 + 원인 분석 |
